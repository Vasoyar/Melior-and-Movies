...
Миграции и схема БД
- [x] бд заполняется приложением без ручного создания таблиц. Схема описана миграциями (Flyway/Liquibase) и накатывается автоматически при старте.
...
- [x]  Неверные данные → 400 с понятным телом ошибки.
...
- [x] Логи содержат время, уровень, краткое описание события/ошибки.
- [x] Уровень логирования настраивается в конфигурации приложения.
...
Минимальные тесты:
- [x] 3–5 unit-тестов ключевой бизнес-логики,
- [ ] 1–2 интеграционных теста с БД (H2/Testcontainers).
- [x] Простая авторизация: регистрация/логин и защищенный эндпоинт, который без токена/сессии возвращает 401/403.
- [x] Для списков есть базовая фильтрация/пагинация или лимит.
- [x] Короткая демо-роль: предзаполненный «демо-пользователь» для быстрого входа.

## Для успешного запуска и сборки необходимы:

| Компонент            | Версия / Примечание                                |
|----------------------|----------------------------------------------------|
| **Java**             | 17 (Adoptium, Oracle, Amazon Corretto – любой)     |
| **Maven**            | 3.8+ (или используйте встроенный Maven Wrapper)    |
